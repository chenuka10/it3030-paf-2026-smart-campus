...
export default function Profile() {
  ...
  const openEdit = () => {
    setForm({ name: user.name||'', phone: user.phone||'', bio: user.bio||'', department: user.department||'' });
    setErrors({});
    setEditing(true);
  };
            ...
            <div style={s.formGrid}>
              <FF label="Full Name" required error={errors.name}>
                <input style={{ ...s.input, ...(errors.name ? s.inputErr : {}) }}
                  value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Your full name" />
              </FF>
              <FF label="Phone Number" error={errors.phone}>
                <input style={{ ...s.input, ...(errors.phone ? s.inputErr : {}) }}
                  value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+94 77 123 4567" />
              </FF>
              <FF label="Department" style={{ gridColumn: 'span 2' }}>
                <input style={s.input}
                  value={form.department} onChange={e => setForm(f => ({ ...f, department: e.target.value }))} placeholder="e.g. Computer Science…" />
              </FF>
              <FF label={`Bio (${(form.bio||'').length}/300)`} error={errors.bio} style={{ gridColumn: 'span 2' }}>
                <textarea style={{ ...s.input, ...s.textarea, ...(errors.bio ? s.inputErr : {}) }}
                  value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))}
                  placeholder="Tell us about yourself…" rows={3} maxLength={300} />
              </FF>
            </div>
...
function FF({ label, required, error, children, style }) {
  return (
...
const s = {
  ...
  input:      { background: 'rgba(5,11,24,0.8)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 10, padding: '11px 14px', color: '#f0f6ff', fontSize: 14, fontFamily: "'DM Sans',sans-serif", outline: 'none', width: '100%', boxSizing: 'border-box' },
