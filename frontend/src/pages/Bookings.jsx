...
const isUpcoming = (dateString, endTimeString) => {
  const bookingEnd = getBookingEndDateTime(dateString, endTimeString);
...
export default function Bookings() {
  ...
  const checkedInBookings = useMemo(() => {
    return bookings.filter((booking) => booking.checkedIn);
  }, [bookings]);

  const missedCheckInBookings = useMemo(() => {
    return bookings.filter((booking) => isPastWithoutCheckIn(booking));
  }, [bookings]);
      ...
      <div className="max-w-[1180px] mx-auto px-6 py-9">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-7">
          ...
          <StatCard
            ...
            sub="Successfully checked in"
            icon="◎"
            tint="linear-gradient(135deg, rgba(111,143,114,0.20), rgba(255,255,255,0.7))"
          />
          <StatCard
            label="Missed Check-In"
            value={missedCheckInBookings.length}
            sub="Passed without check-in"
            icon="!"
            tint="linear-gradient(135deg, rgba(224,122,95,0.16), rgba(255,255,255,0.7))"
          />
        </div>

        {missedCheckInBookings.length > 0 && (
          <div className="mb-7 rounded-[14px] border border-ui-danger/20 bg-ui-danger/8 px-5 py-4">
            <div className="text-[12px] font-bold text-ui-danger mb-1">
              Attention Needed
            </div>
            <div className="text-[14px] text-ui-muted leading-[1.7]">
              You have {missedCheckInBookings.length} approved booking(s) whose
              time has already passed without a recorded check-in. Open the details
              modal to review them.
            </div>
          </div>
        )}
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {!loading && upcomingBookings.length > 0 ? (
                ...
                <EmptyMiniState
                  text={loading ? 'Loading upcoming bookings...' : 'No upcoming bookings yet.'}
                />
              )}
            </div>
          </div>
              ...
              {!loading && recentBookings.length > 0 ? (
                recentBookings.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className={`bg-ui-base border rounded-[14px] px-4 py-4 ${
                        missed
                          ? 'border-ui-danger/20'
                          : checkedIn
                          ? 'border-ui-green/20'
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="text-[12px] text-ui-dim mt-1">
                            {fmtDate(booking.bookingDate)} • {fmtTime(booking.startTime)} -{' '}
                        ...
                        <div className="flex flex-col gap-1 items-end">
                          ...
                          {checkedIn && (
                            <span className="text-[10px] font-bold tracking-[0.08em] px-[9px] py-[4px] rounded-[7px] border uppercase font-mono bg-ui-green/10 text-ui-green border-ui-green/25">
                              CHECKED IN
                            </span>
                          )}

                          {missed && (
                            <span className="text-[10px] font-bold tracking-[0.08em] px-[9px] py-[4px] rounded-[7px] border uppercase font-mono bg-ui-danger/10 text-ui-danger border-ui-danger/25">
                              MISSED CHECK-IN
                            </span>
                          )}
                        </div>
                ...
                <EmptyMiniState
                  text={loading ? 'Loading recent activity...' : 'No recent bookings yet.'}
                />
              )}
            </div>
          </div>
...
function BookingCard({ booking, onCancel, onViewDetails, cancelling }) {
  ...
  return (
    <div
      className={`card ${
        missed ? 'border border-ui-danger/20' : checkedIn ? 'border border-ui-green/20' : ''
      }`}
    >
      <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5">
        <div className="flex-1 min-w-0">
          ...
          {checkedIn && (
            <div className="mb-4 rounded-[12px] border border-ui-green/20 bg-ui-green/8 px-4 py-3">
              <div className="text-[12px] font-bold text-ui-green">Booking Completed</div>
              <div className="text-[13px] text-ui-muted mt-1">
                This booking was successfully checked in.
              </div>
            </div>
          )}

          {missed && (
            <div className="mb-4 rounded-[12px] border border-ui-danger/20 bg-ui-danger/8 px-4 py-3">
              <div className="text-[12px] font-bold text-ui-danger">Missed Check-In</div>
              <div className="text-[13px] text-ui-muted mt-1">
                The booking time has already passed and no check-in was recorded.
              </div>
            </div>
          )}
